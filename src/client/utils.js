...
export const analyzeDataWithMoneyLimit = (
  data,
  limitOfMoney
) => {
  let moneyThreshold = limitOfMoney;
    ...
    for (let k = 0; k < data.length; k++) {
      ...
      if (
        ...
        if (trade.taken) {
          ...
          currentInvestedAmount +=
            stockTrades[trade.stockName].numberOfShares * stockTrades[trade.stockName].buyPrice;
          totalMoneyUsedForBuying +=
            stockTrades[trade.stockName].remainingCorpus;
          stockTrades[trade.stockName].tradesTaken += 1;
          onGoingTrades += 1;
        ...
        const profitOrLoss = parseInt(trade["Profit NONE"])*stockTrades[trade.stockName].numberOfShares;
        //   console.log("profitOrLoss", profitOrLoss);
        stockTrades[trade.stockName].remainingCorpus += profitOrLoss;
